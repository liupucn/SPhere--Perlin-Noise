...
void CSphere::Draw(CDC* pDC, CZBuffer* pZBuffer)
{
	...
	for (int i = 0;i < 180;i += deltAlpha)
	{
		...
		for (int j = 0;j < 360;j += deltBeta)
		{
			...
			CTransform tran;//三维变换
			tran.SetMat(Vertex, 4);//变换矩阵
			tran.Scale(nScalar, nScalar, nScalar);//缩放比例
			tran.RotateX(Alpha);//绕x轴旋转
			tran.RotateY(Beta);//绕y轴旋转
			DrawFacet(pDC, pZBuffer, Vertex);//绘制平面片
		}
	}
}

void CSphere::DrawFacet(CDC* pDC, CZBuffer* pZBuffer, CP3* P)//绘制小面
{
	CP3 Point4[4];//投影点
	CVector3 Normal4[4];//顶点法向量
	CVector3 ViewVector(P[0], projection.GetEye());// 面的视矢量
	...
	if (DotProduct(ViewVector, FacetNormal) >= 0)//背面剔除
	{
		for (int nPoint = 0;nPoint < 4;nPoint++)
		{
			...
			Normal4[nPoint] = CVector3(P[nPoint]).Normalize();
		}
		pZBuffer->SetPoint(Point4[0], Point4[1], Point4[2], Normal4[0], Normal4[1], Normal4[2], P[0], P[1], P[2]);//下三角形
		pZBuffer->PhongShading(pDC, projection.GetEye(), pLight, pMaterial);
		pZBuffer->SetPoint(Point4[0], Point4[2], Point4[3], Normal4[0], Normal4[2], Normal4[3], P[0], P[2], P[3]);//上三角形
		pZBuffer->PhongShading(pDC, projection.GetEye(), pLight, pMaterial);
	}
}
